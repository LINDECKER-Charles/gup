...
import { DockerImagesProvider } from "../../src/providers/containers/docker-images.js";
...
describe("docker-images provider", () => {
  it("isAvailable false when docker binary is missing", async () => {
    commandExistsMock.mockResolvedValueOnce(false);
    await expect(new DockerImagesProvider().isAvailable()).resolves.toBe(false);
    expect(runMock).not.toHaveBeenCalled();
  });

  it("isAvailable false when docker info probe fails", async () => {
    commandExistsMock.mockResolvedValueOnce(true);
    runMock.mockResolvedValueOnce(mkRun("", true));
    await expect(new DockerImagesProvider().isAvailable()).resolves.toBe(false);
  });

  it("isAvailable true when docker info succeeds", async () => {
    commandExistsMock.mockResolvedValueOnce(true);
    runMock.mockResolvedValueOnce(mkRun("24.0.5"));
    await expect(new DockerImagesProvider().isAvailable()).resolves.toBe(true);
  });

  it("listOutdated returns [] when docker image ls fails", async () => {
    runMock.mockResolvedValueOnce(mkRun("", true));
    await expect(new DockerImagesProvider().listOutdated()).resolves.toEqual([]);
  });

  it("listOutdated returns [] when output is empty", async () => {
    runMock.mockResolvedValueOnce(mkRun(""));
    await expect(new DockerImagesProvider().listOutdated()).resolves.toEqual([]);
  });

  it("listOutdated parses tagged images, skips <none>, dedupes, and surfaces refresh candidates", async () => {
    const stdout = [
      "alpine\t3.18\tabcdef0123456789\t7.4MB",
      "<none>\t<none>\tdeadbeefcafe1234\t100MB", // skipped
      "redis\t<none>\tcafedeadbeef5678\t50MB", // skipped
      "<none>\t7.0\tffff00001111aaaa\t60MB", // skipped (repo is <none>)
      "  ", // blank
  ...
  it("update returns success when docker pull succeeds", async () => {
    runInheritMock.mockResolvedValueOnce(mkRun(""));
    await expect(new DockerImagesProvider().update("alpine:3.18")).resolves.toEqual({
      id: "alpine:3.18",
      success: true,
    });
    expect(runInheritMock).toHaveBeenCalledWith("docker", ["pull", "alpine:3.18"]);
  });

  it("update returns failure when docker pull fails", async () => {
  ...
  it("updateAll iterates all packages", async () => {
    runInheritMock
      .mockResolvedValueOnce(mkRun(""))
      .mockResolvedValueOnce(mkRun("", true));
    const res = await new DockerImagesProvider().updateAll([
      { id: "alpine:3.18", name: "alpine:3.18", current: "x", latest: "pull" } as never,
      { id: "nginx:latest", name: "nginx:latest", current: "y", latest: "pull" } as never,
    ]);
    expect(res).toEqual([
      { id: "alpine:3.18", success: true },
      { id: "nginx:latest", success: false },
    ]);
    expect(runInheritMock).toHaveBeenCalledTimes(2);
  });

  it("updateAll handles empty packages", async () => {
    await expect(new DockerImagesProvider().updateAll([])).resolves.toEqual([]);
